...
export function ConfirmDialog({ isOpen, title, description, confirmLabel = '确认', onClose, onConfirm }: {
    isOpen: boolean;
    title: string;
    description: string;
    confirmLabel?: string;
    ...
    return (
        <Dialog open={isOpen} onClose={onClose} labelledBy="confirm-dialog-title" describedBy="confirm-dialog-description" alert className="w-full max-w-md rounded-xl border border-border bg-background shadow-2xl">
            ...
            <div className="flex justify-end gap-3 px-6 py-4"><Button variant="outline" onClick={onClose}>取消</Button><Button className="bg-red-600 text-white hover:bg-red-700" onClick={onConfirm}>{confirmLabel}</Button></div>
        </Dialog>
    );
}
